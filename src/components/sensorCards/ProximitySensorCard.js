...
class ProximitySensorCard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      switchActive: true,
  ...
  onNewMqttMessage = (topic, message) => {
    if(topic === this.state.mqttTopic) 
    {
      ...
      this.setState({ proximityValueData: [ ...this.state.proximityValueData, {"value": messageData.close ? 1 : 0} ] })
    }
  };
  ...
  render() {
    ...
    let chartDataSource = {
      ...
      "dataset": [
        {
          "seriesname": "Close",
          "data": this.state.proximityValueData
        }
      ]
    }
...
ProximitySensorCard.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(ProximitySensorCard);