...
class TiltSensorCard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      switchActive: true,
  ...
  onNewMqttMessage = (topic, message) => {
    if(topic === this.state.mqttTopic) 
    {
      ...
      this.setState({ xRotationData: [ ...this.state.xRotationData, {"value": messageData.xRotation} ] })
      this.setState({ yRotationData: [ ...this.state.yRotationData, {"value": messageData.yRotation} ] })
    }
  };
  ...
  render() {
    ...
    let chartDataSource = {
      ...
      "dataset": [
        {
          "seriesname": "xRotation",
          "data": this.state.xRotationData
        },
        {
          "seriesname": "YRotation",
          "data": this.state.yRotationData
        }
      ]
    }
...
TiltSensorCard.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(TiltSensorCard);