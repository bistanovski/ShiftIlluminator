...
class LightSensorCard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      switchActive: true,
  ...
  onNewMqttMessage = (topic, message) => {
    if(topic === this.state.mqttTopic) 
    {
      ...
      this.setState({ luxValueData: [ ...this.state.luxValueData, {"value": messageData.luxValue} ] })
    }
  };
...
export default withStyles(styles)(LightSensorCard);